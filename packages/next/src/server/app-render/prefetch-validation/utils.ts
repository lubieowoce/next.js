...
export function createDebugFriendlyNodeStreamFromChunks(
  ...
  if (process.env.NEXT_RUNTIME === 'edge') {
    throw new InvariantError(
      'createDebugFriendlyNodeStreamFromChunks cannot be used in the edge runtime'
    )
  } else {
    ...
    signal.addEventListener(
      ...
      () => {
        ...
        while (nextIndex < allChunks.length) {
          // console.log(new TextDecoder().decode(allChunks[nextIndex]))
