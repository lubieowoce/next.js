...
function decodeReplyFromBusboy(busboyStream, webpackMap, options) {
  const response = createResponse(webpackMap, '', options ? options.temporaryReferences : undefined);
  let pendingFiles = 0;
  const queuedFields = [];
...
function decodeReply(body, webpackMap, options) {
  ...
  const response = createResponse(webpackMap, '', options ? options.temporaryReferences : undefined, body);
