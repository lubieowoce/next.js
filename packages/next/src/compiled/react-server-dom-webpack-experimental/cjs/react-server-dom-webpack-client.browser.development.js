...
var ReactVersion = '19.3.0-experimental-ed524316-20260123';
...
function getModuleDebugInfo(metadata) {
  ...
  while (i < chunks.length) {
    var chunkId = chunks[i++];
    chunks[i++];
    addChunkDebugInfo(debugInfo, chunkId);
  }
  return debugInfo;
}
...
function addChunkDebugInfo(target, chunkId, filename) {
  var ioInfo = chunkIOInfoCache.get(chunkId);
  if (ioInfo === undefined) {
...
var rendererPackageName = 'react-server-dom-webpack';
